...
class MyDataloader(data.Dataset):
    ...
    def __init__(self, root, type, sparsifier=None, modality='rgb',subset='train'):
    ...
    def __getraw__(self, index):
        ...
        with open(image_path_city(self.images_root, filename), 'rb') as f:
            #rgb = load_image(f).convert('RGB')
            rgb = np.asarray(load_image(f).convert('RGB'))
        with open(image_path_city(self.labels_root, filenameGt), 'rb') as f:
            depth = np.asarray(load_image(f).convert('L'))
            #depth = load_image(f).convert('P')
