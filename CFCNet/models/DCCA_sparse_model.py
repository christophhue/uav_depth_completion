...
class DCCASparseModel(BaseModel):
    ...
    def initialize(self, opt, dataset):
        ...
        #self.MSE = DCCA_sparse_networks.TrMAELoss()
        #self.bMSE = DCCA_sparse_networks.EdgeMaskedMSELoss()
        self.SMOOTH = DCCA_sparse_networks.SmoothLoss()
        #self.SSIM = DCCA_sparse_networks.SSIM()
        ...
        if opt.pretrained:
            ...
            od = OrderedDict(sorted(new_state_dict.items()))
        ...
        if self.isTrain:
            ...
            self.optimizers = []
            self.optimizer_DCCASparseNet = torch.optim.Adam(self.netDCCASparseNet.parameters(), lr=opt.lr, weight_decay=opt.weight_decay)
            if opt.arch == "VGG":
                self.optimizer_DCCASparseNet = torch.optim.SGD(self.netDCCASparseNet.parameters(), lr=opt.lr,
                                                               momentum=opt.momentum, weight_decay=opt.weight_decay)
            self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer_DCCASparseNet, step_size=5, gamma=0.1)
    ...
    def get_loss(self):
        ...
        #self.loss_mse = self.loss_mse + self.bMSE(self.depth_est,self.depth_image)
        self.loss_smooth = self.SMOOTH(self.depth_est)
        self.loss_transform = self.TransformLoss(self.x_trans, self.x_dataview)
        #self.loss_mse = 0.85 * self.SSIM(self.depth_est, self.depth_image).mean() + (1 - 0.85) * torch.abs(self.depth_est - self.depth_image).mean()
        self.loss_total = 1 * self.loss_mse + 1* self.loss_dcca + 1* self.loss_transform
    ...
    def get_log(self):
        return self.loss_mse, self.loss_dcca, self.loss_total, self.depth_est, self.rgb_image
...
Lg10={result.lg10:.4f}({average.lg10:.4f})'.format(result=self.result, average=self.average.average())
    ...
    def lr_schedule(self,epoch):
        ...
        print('used learning rate = %.7f' % lr)
    ...
    def optimize_parameters(self):
        ...
        # update DCCAnet
        self.backward()
        #plot_grad_flow(self.netDCCASparseNet.named_parameters())
...
class Result(object):
    ...
    def evaluate(self, output, target):
        ...
        #target = target * scale
        valid_mask = target>0
        valid_mask = (target > 0) & (target <= 500)
