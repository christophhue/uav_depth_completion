...
class DCCASparsevalModel(BaseModel):
    def name(self):
    ...
    def initialize(self, opt, dataset):
        ...
        if self.isTrain:
            ...
            #self.lambda1 = lambda epoch: pow((1-((epoch-1)/20)),0.9)
            #self.scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer_DCCASparseNet, lr_lambda=self.lambda1)
    ...
    def set_new_input(self, input,target,scale):
    ...
    def test_depth_evaluation(self):
        self.test_result.evaluate(self.depth_est.data, self.depth_image.data,self.scale.data)
...
class Result(object):
    ...
    def evaluate(self, output, target,scale):
        output = output * scale
        target = target * scale
        valid_mask = target>0
