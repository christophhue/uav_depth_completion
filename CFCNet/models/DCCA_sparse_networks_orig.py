...
def convt2(in_channels,kernel_size):
    stride = 2
    padding = (kernel_size - 1) // 2
    output_padding = kernel_size % 2
    # assert -2 - 2 * padding + kernel_size + output_padding == 0, "deconv parameters incorrect
    module_name2 = "deconv{}".format(kernel_size)
    return nn.Sequential(collections.OrderedDict([
        (module_name2, nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size,
                                          stride, padding, output_padding, bias=False)),
        ('batchnorm', nn.BatchNorm2d(in_channels // 2)),
        ('lrelu', nn.LeakyReLU(negative_slope=2e-3,inplace=True)),
    ]))
...
class DCCASparsenetGenerator(nn.Module):
    def __init__(self, rgb_enc=True, depth_enc=True, depth_dec=True):
        ...
        if  depth_dec :
            ...
            self.bilinear = nn.Upsample((365, 547), mode='bilinear', align_corners=True)
    ...
    def forward(self, sparse_rgb,sparse_d,mask,rgb,d):
        ...
        x = self.conv3(x)
        #print("shape before upsampling", x.shape)
        depth_est = self.bilinear(x)
        #depth_est = self.conv3(depth_est)
        #depth_est = torch.tanh(depth_est)
       # depth_est = torch.clamp(depth_est, min=0)
...
class EdgeMaskedMSELoss(nn.Module):
    def __init__(self):
        super(EdgeMaskedMSELoss, self).__init__()
