...
class Decoder(nn.Module):
    ...
    def forward(self, inp):
    	x, seg = inp
    	x = self.layer1(x)
    	x = self.layer1_1((x,seg))
    	x = self.layer1_2(x)
    	x = self.layer2(x)
    	x = self.layer2_1((x,seg))
    	x = self.layer2_2(x)
    	x = self.layer3(x)
    	x = self.layer3_1((x,seg))
    	x = self.layer3_2(x)
    	x = self.layer4(x)
    	x = self.layer4_1((x,seg))
    	x = self.layer4_2(x)
    	return x
    ...
    def __init__(self, in_channels, kernel_size):
        ...
        def convt(in_channels):
            ...
            return nn.Sequential(collections.OrderedDict([
                  ...
                  #('SPADE', SPADE(in_channels,in_channels//2))
                  #('relu',      nn.ReLU(inplace=True)),
                ]))
        self.layer1 = convt(in_channels)
        self.layer1_1 =SPADE(in_channels//2,4)
        self.layer1_2 = nn.ReLU(inplace=True)
        self.layer2 = convt(in_channels // 2)
        self.layer2_1 = SPADE(256,4)
        self.layer2_2 = nn.ReLU(inplace=True)
        self.layer3 = convt(in_channels // (2 ** 2))
        self.layer3_1 = SPADE(128,4)
        self.layer3_2 = nn.ReLU(inplace=True)
        self.layer4 = convt(in_channels // (2 ** 3))
        self.layer4_1 = SPADE(64,4)
        self.layer4_2 = nn.ReLU(inplace=True)
...
class DCCASparsenetGenerator(nn.Module):
	def __init__(self, rgb_enc=True, depth_enc=True, depth_dec=True):
		...
		if  depth_dec :
			...
			#self.bilinear = nn.Upsample((224,224), mode='bilinear', align_corners=True)
			self.bilinear = nn.Upsample((365, 547), mode='nearest')
	...
	def forward(self, sparse_rgb,sparse_d,mask,rgb,d):
		...
		########  DECODER  ########
		sp = torch.cat((sparse_d,sparse_rgb), 1)
		x = self.decoder((torch.cat((x_dataview,xm_trans),1),sp))
		x = self.conv3(x)
