...
class Gudi_UpProj_Block(nn.Module):
    ...
    def _up_pooling(self, x, scale):
        ...
        #print("up pooling inp shape", x.shape)
        if self.oheight !=0 and self.owidth !=0:
            x = x[:,:,0:self.oheight, 0:self.owidth]
        mask = torch.zeros_like(x)
        for h in range(0, self.oheight-1, 2):
            for w in range(0, self.owidth-1, 2):
                mask[:,:,h,w] = 1
        x = torch.mul(mask, x)
        return x
...
class ResNet(nn.Module):
    def __init__(self, block, layers, up_proj_block, cspn_config=None):
        ...
        self.gud_up_proj_layer1 = self._make_gud_up_conv_layer(Gudi_UpProj_Block, 2048, 1024, 22, 32)
        self.gud_up_proj_layer2 = self._make_gud_up_conv_layer(Gudi_UpProj_Block_Cat, 1024, 512, 44, 64)
        self.gud_up_proj_layer3 = self._make_gud_up_conv_layer(Gudi_UpProj_Block_Cat, 512, 256, 88, 128)
        self.gud_up_proj_layer4 = self._make_gud_up_conv_layer(Gudi_UpProj_Block_Cat, 256, 64, 176, 256)
        self.gud_up_proj_layer5 = self._make_gud_up_conv_layer(Simple_Gudi_UpConv_Block_Last_Layer, 64, 1, 352, 512)
        self.gud_up_proj_layer6 = self._make_gud_up_conv_layer(Simple_Gudi_UpConv_Block_Last_Layer, 64, 8, 352, 512)
